...
export async function searchChatMessages(
  ...
  if (directIds.length > 0) {
    let q = supabase
      .from("messages")
      .select(
        "id, conversation_id, sender_id, content, created_at, audio_url, file_name, transcript_text, transcript_status"
      )
  ...
  if (topicIds.length > 0) {
    let q = supabase
      ...
      .select(
        "id, topic_id, sender_id, content, created_at, audio_url, file_name"
      )
 ...
 */
export function downloadChatPdf(
  hits: ChatSearchHit[],
  filter: ChatFilterInput,
  meta: { exportedBy: string; version?: string }
): void {
  ...
  const lines = [
    ...
    filter.dateFrom || filter.dateTo
      ...
      : "Periodo: tutti",
    filter.query.trim() ? `Ricerca: ${filter.query.trim()}` : null,
  ...
  // Export chronological (oldest first)
  ...
  for (const hit of ordered) {
    ...
    if (filter.includeText && hit.content.trim()) bodyParts.push(hit.content.trim());
    if (filter.includeTranscripts && hit.transcriptText?.trim()) {
      bodyParts.push(`[Trascrizione] ${hit.transcriptText.trim()}`);
    }
    if (filter.includeAttachments && hit.fileName) {
