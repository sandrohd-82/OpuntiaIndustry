...
import { setPasswordResetAction } from "@/app/actions/password-reset";
...
export function RecuperoPasswordImpostaForm({ token, email }: Props) {
  const router = useRouter();
  const [state, formAction, pending] = useActionState(
    async (_prev: State, formData: FormData) => {
      return setPasswordResetAction(formData);
    },
    initial
  );
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      >
        {pending ? "Salvataggio…" : "Imposta la nuova password"}
      </button>
    </form>
  );
}
