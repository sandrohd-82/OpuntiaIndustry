...
export function AnagraficaTimelineSyncBar({
  ...
  return (
    <div className="sm:col-span-2 rounded-xl border-2 border-emerald-400 bg-emerald-50 px-4 py-3">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        ...
        >
          {attiva ? "Mantieni sincronizzato" : "Sincronizzazione disattivata"}
        </button>
        <button
          type="button"
          onClick={() => setForceOpen(true)}
          className="rounded-lg border-2 border-emerald-800 bg-white px-3 py-2 text-sm font-semibold text-emerald-950 hover:bg-emerald-100"
        >
          Forza nuova sincronizzazione
        </button>
        <p className="text-xs text-emerald-900">
          {attiva
            ? "Attivo di default: mail, promemoria, attività e documenti si collegano alla scheda. Solo un Super Admin può disattivarlo (doppia conferma)."
            : "In pausa: le nuove mail non si collegano da sole."}
        </p>
      </div>
      {info ? (
        <p className="mt-1 text-xs text-emerald-900">{info}</p>
      ) : null}
      {error ? (
            ...
            {pauseStep === 1 ? (
              <>
                <p className="mt-2 text-sm text-slate-700">
                  Prima conferma: le mail e i promemoria non si collegheranno più
                  da soli a «{aziendaLabel}». Serve il consenso Super Admin.
                </p>
